...
export class CompaniesService {
  ...
  addCompany(emp: Empresa): Observable<number> {
    return this.http.post<number>(`${this.url}/addCompany.php`, {
      empresa: emp,
    });
  }

  public updateCompany(emp: Empresa): Observable<number> {
    return this.http.post<number>(`${this.url}/updateCompany.php`, {
      empresa: emp,
    });
  }
  ...
  public getApi(): Observable<string> {
    return new Observable((observer) => {
      observer.next(environment.apiMap);
      observer.complete();
    });
  }
  ...
  public toAbleDisableCompany(emp: Empresa): Observable<number> {
    return this.http.post<number>(`${this.url}/ableDisableCompany.php`, {
      empresa: emp,
    });
  }
}
