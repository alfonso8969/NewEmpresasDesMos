...
import {
  ...
  SortEventEmail,
  compare,
} from 'src/app/events/sortable-headers.emails.directive';
...
export class TechnicalEmailsComponent implements OnInit {
  ...
  public getEmails(): void {
    this.load = true;
    this.loadEmails();
    this.emailsStorage = this.emails;
        ...
        if (result != null) {
          result.forEach((email: Email) => {
            ...
            if (email.label.toLowerCase().includes('inscription')) {
              email.bodyText = this.prepareBody(
                email,
                email.idEmail!,
                email.bodyText,
              );
          ...
          if (
            this.emailsStorage === null ||
            this.emailsStorage === undefined ||
            this.emailsStorage.length === 0
          ) {
  ...
  private getEmailsResponse(): void {
    this.emailService.getEmailsResponse().subscribe({
      ...
      complete: () => {
        console.log(
          'Se consiguieron los emails enviados:',
          this.emailsResponse,
        );
      },
    });
  ...
  private compareArray(emailsStorage: Email[], result: Email[]) {
    result.forEach((emailResult: Email) => {
      if (
        emailsStorage.findIndex(
          (emailStorage: Email) => emailStorage.idEmail === emailResult.idEmail,
        ) < 0
      ) {
  ...
  public checkAll(read: boolean): void {
    ...
    this.emails.forEach((email) => {
      switch (ct.name) {
        case 'isInBox':
          if (!email.deleted && !email.answered) email.unread = !read;
          break;
        case 'isInFavorites':
        ...
        case 'isInTrash':
          ...
          break;
        case 'isInLabels':
          if (ct.label == email.label) email.unread = !read;
          break;
  ...
  public filterInBox(): void {
    ...
    this.emails = this.emailsTmp.filter(
      (email) => !email.deleted && !email.answered,
    );
  }
  ...
  private setTotals(emails: Email[]): void {
    this.emails = emails.filter(
      (email) => Boolean(!email.deleted) && Boolean(!email.answered),
    );
    this.emailsTmp = JSON.parse(JSON.stringify(emails));
    this.emailsTotal = emails.length;
    this.emailsUnreadTotal = emails.filter((email) =>
      Boolean(!email.unread),
    ).length;
    this.emailsReadTotal = emails.filter((email) =>
      Boolean(email.unread),
    ).length;
    this.emailsFavoritesTotal = emails.filter((email) =>
      Boolean(email.favorite != !1),
    ).length;
    this.emailsDeletedTotal = emails.filter((email) =>
      Boolean(email.deleted),
    ).length;
    this.emailsSendedTotal = emails.filter((email) =>
      Boolean(email.answered),
    ).length;

    localStorage.setItem('emailsTotal', this.emailsTotal.toString());
    localStorage.setItem(
      'emailsUnreadTotal',
      this.emailsUnreadTotal.toString(),
    );
    localStorage.setItem('emailsReadTotal', this.emailsReadTotal.toString());
    localStorage.setItem(
      'emailsFavoritesTotal',
      this.emailsFavoritesTotal.toString(),
    );
    localStorage.setItem(
      'emailsDeletedTotal',
      this.emailsDeletedTotal.toString(),
    );
    localStorage.setItem(
      'emailsSendedTotal',
      this.emailsSendedTotal.toString(),
    );
    /*  try {
      localStorage.setItem('emails', JSON.stringify(emails));
    } catch (error) {
      alert(
        'Error al guardar los emails en el almacenamiento local. Es posible que se haya superado la cuota de almacenamiento.',
      );
      console.error(
        'Error al guardar los emails en el almacenamiento local:',
        error,
      );
    } */
    this.saveEmails();
  ...
  private prepareBody(email: Email, idEmail: number, text: string): string {
    ...
    this.formInscription.title = body
      .substring(intForm + 'Formulario:'.length, body.indexOf('Fecha:'))
      .trim();
    let intDate = body.indexOf('Fecha:');
    this.formInscription.date = body
      .substring(intDate + 'Fecha:'.length, body.indexOf('Nombre'))
      .trim();
    let intUserName = body.indexOf('Nombre');
    this.formInscription.userName = body
      .substring(intUserName + 'Nombre'.length, body.indexOf('Apellido'))
      .trim();
    let intUserLastName = body.indexOf('Apellido');
    this.formInscription.userLastName = body
      .substring(intUserLastName + 'Apellido'.length, body.indexOf('Email'))
      .trim();
    let intUserEmail = body.indexOf('Email');
    this.formInscription.userEmail = body
      .substring(intUserEmail + 'Email'.length, body.indexOf('Código de área'))
      .trim();
    let intUserPhoneAreaCode = body.indexOf('Código de área');
    this.formInscription.userPhoneAreaCode = body
      .substring(
        intUserPhoneAreaCode + 'Código de área'.length,
        body.indexOf('Número de teléfono'),
      )
      .trim();
    ...
    this.formInscription.userPhone = Number(
      body
        .substring(
          intUserPhone + 'Número de teléfono'.length,
          body.indexOf('Código de área'),
        )
        .trim(),
    );
    let intUserOtherPhoneAreaCode = body.indexOf('Código de área');
    this.formInscription.userOtherPhoneAreaCode = body
      .substring(
        intUserOtherPhoneAreaCode + 'Código de área'.length,
        body.indexOf('Otro teléfono'),
      )
      .trim();
    let intUserOtherPhone = body.indexOf('Otro teléfono');
    this.formInscription.userOtherPhone = Number(
      body
        .substring(
          intUserOtherPhone + 'Otro teléfono'.length,
          body.indexOf('Dirección de la calle'),
        )
        .trim(),
    );
    let intUserAddress = body.indexOf('Dirección de la calle');
    this.formInscription.userAddress = body
      .substring(
        intUserAddress + 'Dirección de la calle'.length,
        body.indexOf('Dirección de la calle Línea 2'),
      )
      .trim();
    let intUserAddress2 = body.indexOf('Dirección de la calle Línea 2');
    this.formInscription.userAddress2 = body
      .substring(
        intUserAddress2 + 'Dirección de la calle Línea 2'.length,
        body.indexOf('Ciudad'),
      )
      .trim();
    let intUserCity = body.indexOf('Ciudad');
    this.formInscription.userCity = body
      .substring(
        intUserCity + 'Ciudad'.length,
        body.indexOf('Estado / Provincia'),
      )
      .trim();
    let intUserRegion = body.indexOf('Estado / Provincia');
    this.formInscription.userRegion = body
      .substring(
        intUserRegion + 'Estado / Provincia'.length,
        body.indexOf('Código postal'),
      )
      .trim();
    let intUserPostalCode = body.indexOf('Código postal');
    this.formInscription.userPostalCode = Number(
      body
        .substring(
          intUserPostalCode + 'Código postal'.length,
          body.indexOf('Nombre de la empresa'),
        )
        .trim(),
    );
    ...
    this.formInscription.companyName = body
      .substring(
        intCompanyName + 'Nombre de la empresa'.length,
        body.indexOf('CIF'),
      )
      .trim();
    let intCIF = body.indexOf('CIF');
    this.formInscription.CIF = body
      .substring(intCIF + 'CIF'.length, body.indexOf('Email'))
      .trim();
    let intCompanyEmail = body.indexOf('Email');
    this.formInscription.companyEmail = body
      .substring(intCompanyEmail + 'Email'.length, body.indexOf('Sector'))
      .trim();
    let intCompanySector = body.indexOf('Sector');
    this.formInscription.companySector = body
      .substring(
        intCompanySector + 'Sector'.length,
        body.indexOf('Código de área'),
      )
      .trim();
    let intCompanyPhoneAreaCode = body.indexOf('Código de área');
    this.formInscription.companyPhoneAreaCode = body
      .substring(
        intCompanyPhoneAreaCode + 'Código de área'.length,
        body.indexOf('Número de teléfono'),
      )
      .trim();
    ...
    this.formInscription.companyPhone = Number(
      body
        .substring(
          intCompanyPhone + 'Número de teléfono'.length,
          body.indexOf('Código de área'),
        )
        .trim(),
    );
    let intCompanyOtherPhoneAreaCode = body.indexOf('Código de área');
    this.formInscription.companyOtherPhoneAreaCode = body
      .substring(
        intCompanyOtherPhoneAreaCode + 'Código de área'.length,
        body.indexOf('Otro teléfono'),
      )
      .trim();
    let intCompanyOtherPhone = body.indexOf('Otro teléfono');
    this.formInscription.companyOtherPhone = Number(
      body
        .substring(
          intCompanyOtherPhone + 'Otro teléfono'.length,
          body.indexOf('Persona contacto'),
        )
        .trim(),
    );
    let intCompanyContactPerson = body.indexOf('Persona contacto');
    this.formInscription.companyContactPerson = body
      .substring(
        intCompanyContactPerson + 'Persona contacto'.length,
        body.indexOf('Año instalación'),
      )
      .trim();
    let intCompanyYear = body.indexOf('Año instalación');
    this.formInscription.companyYear = Number(
      body
        .substring(
          intCompanyYear + 'Año instalación'.length,
          body.indexOf('Nº Trabajadores'),
        )
        .trim(),
    );
    let intCompanyWorks = body.indexOf('Nº Trabajadores');
    this.formInscription.companyWorks = Number(
      body
        .substring(
          intCompanyWorks + 'Nº Trabajadores'.length,
          body.indexOf('Dirección de la calle'),
        )
        .trim(),
    );
    let intCompanyAddress = body.indexOf('Dirección de la calle');
    this.formInscription.companyAddress = body
      .substring(
        intCompanyAddress + 'Dirección de la calle'.length,
        body.indexOf('Dirección de la calle Línea 2'),
      )
      .trim();
    let intCompanyAddress2 = body.indexOf('Dirección de la calle Línea 2');
    this.formInscription.companyAddress2 = body
      .substring(
        intCompanyAddress2 + 'Dirección de la calle Línea 2'.length,
        body.indexOf('Ciudad'),
      )
      .trim();
    let intCompanyCity = body.indexOf('Ciudad');
    this.formInscription.companyCity = body
      .substring(
        intCompanyCity + 'Ciudad'.length,
        body.indexOf('Estado/Provincia'),
      )
      .trim();
    let intCompanyRegion = body.indexOf('Estado/Provincia');
    this.formInscription.companyRegion = body
      .substring(
        intCompanyRegion + 'Estado/Provincia'.length,
        body.indexOf('Código postal'),
      )
      .trim();
    let intCompanyPostalCode = body.indexOf('Código postal');
    this.formInscription.companyPostalCode = Number(
      body
        .substring(
          intCompanyPostalCode + 'Código postal'.length,
          body.indexOf('Distrito'),
        )
        .trim(),
    );
    let intDistrito = body.indexOf('Distrito');
    this.formInscription.companyDistrict = body
      .substring(intDistrito + 'Distrito'.length, body.indexOf('Polígono'))
      .trim();
    let intPoligono = body.indexOf('Polígono');
    this.formInscription.companyWorkArea = body
      .substring(intPoligono + 'Polígono'.length, body.indexOf('Página Web'))
      .trim();
    let intCompanyWeb = body.indexOf('Página Web');
    this.formInscription.companyRedWeb = body
      .substring(intCompanyWeb + 'Página Web'.length, body.indexOf('Twitter'))
      .trim();
    let intCompanyTwitter = body.indexOf('Twitter');
    this.formInscription.companyRedTwitter = body
      .substring(intCompanyTwitter + 'Twitter'.length, body.indexOf('Facebook'))
      .trim();
    let intCompanyFacebook = body.indexOf('Facebook');
    this.formInscription.companyRedFacebook = body
      .substring(
        intCompanyFacebook + 'Facebook'.length,
        body.indexOf('Instagram'),
      )
      .trim();
    let intCompanyInstagram = body.indexOf('Instagram');
    this.formInscription.companyRedInstagram = body
      .substring(
        intCompanyInstagram + 'Instagram'.length,
        body.indexOf('LinkedIn'),
      )
      .trim();
    let intCompanyLinkedin = body.indexOf('LinkedIn');
    this.formInscription.companyRedLinkedin = body
      .substring(intCompanyLinkedin + 'LinkedIn'.length, body.indexOf('TikTok'))
      .trim();
    let intCompanyTikTok = body.indexOf('TikTok');
    this.formInscription.companyRedTikTok = body
      .substring(intCompanyTikTok + 'TikTok'.length, body.indexOf('Fecha'))
      .trim();
    let intTerms = body.indexOf('Términos y condiciones');
    let intFirma = body.indexOf('Firma');
    let terms = body
      .substring(intTerms + 'Términos y condiciones'.length, intFirma)
      .trim();
  ...
  async saveEmails() {
    await this.emailService.setItem('emails', JSON.stringify(this.emails));
  }

  async loadEmails() {
    const data = await this.emailService.getItem('emails');
    if (data) {
      this.emails = JSON.parse(data);
      console.log('Emails recuperados:', this.emails);
    }
  }
}
